...
class RandomNormal(RandomGenerator):
    '''
        RandomNormal class inherits from RandomGenerator and generates connections that are normally distributed
    '''
    ...
    def getEnhancedConnection(self):        # line needs to be continuous...
        '''
            Generate normally distributed connections
        '''
        exist_connections = {s: 0 for s in self.s_list}
        all_possible_connections = {s1:[s2 for s2 in self.s_list if s2 != s1] for s1 in self.s_list}
        connections = []
        # Generate appropriate number of connections based on pre-assigned value
        for s1 in self.s_list:
            ...
            while exist_connections[s1] < self.degree_of_station[s1]:
                ...
                if exist_connections[s2] < self.degree_of_station[s2]:
                    ...
                    connections.append(Connection(s1, s2, random.choice(self.l_list), random.randint(1, 4)))
        self.c_list = connections


    def getNormalDegree(self):
        '''
            Return a numpy array of numbers that are normally distributed by a mean of self.degree and standard deviation of 1
        '''
    ...
    def __getStationDegree(self):
        '''
            Generate a list that specifies the number of stations of each degree (1 < degree < degree * 2 - 1).
        '''
        degree_list = [0] * (self.degree * 2 - 1)
        start_degree = 1.5
        # Iterate through the degree list and increment the number of stations at corresponding index 
    ...
    def assignStationDegree(self):
        ...
        # Example: If there is 5 stations with degree 1, then add 5 random stations to the list at station_of_degree_dict[5]
        ...
        # Convert station_of_degree_dict to a dictionary that describes how many degrees each station has
