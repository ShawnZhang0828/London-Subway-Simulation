...
class RandomGenerator():
    ...
    def genStation(self):
        ...
        for i in range(self.s_num):
            ...
            stations.append(Station(i, 50+random.random(), -0.5+random.random(), 
                            station_name, station_name, random.choice([i/2 for i in range(2, 11)]) , None, None))
    ...
    def genConnection(self):
        ...
        for s1 in all_possible_connections.keys():
            # Create self.degree connections between s1 and other random stations
    ...
    def genStr(self, for_s=1):
        '''
            Return a random station name from station names that have already existed
    ...
    def genStationNames(self, csv_path):
        '''
            Generate all possible station names given a csv path that stores station information
        '''
        station_names = []
        with open(csv_path, newline='') as csvfile:     # open .csv file
    ...
    def genLineNames(self, csv_path):
        '''
            Generate all possible line names given a csv path that stores line information
        '''
        line_names = []
        with open(csv_path, newline='') as csvfile:             # open .csv file
            spamreader = csv.reader(csvfile)
